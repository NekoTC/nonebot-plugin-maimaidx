...
from .maimaidx_model import ChartInfo, PlayInfoDefault, PlayInfoDev, UserInfo, UserInfoDev
...
class DrawBest(ScoreBaseImage):
    ...
    def __init__(self, UserInfo: UserInfo = None, UserInfoDev: UserInfoDev = None, qqid: Optional[Union[int, str]] = None, normal: bool = True) -> None:
        ...
        if normal:
            ...
            self.normal = False
            self.userName = UserInfoDev.nickname
            self.plate = UserInfoDev.plate
            self.addRating = UserInfoDev.additional_rating
            self.Rating = UserInfoDev.rating
            self.dxBest = None
            self.sdBest = UserInfoDev.records
        self.qqid = qqid
    ...
    async def draw(self) -> Image.Image:
        ...
        if self.normal:
            ...
            CusRA = sum([_.ra for _ in self.sdBest])
            self._tb.draw(
                570, 172, 17, 
                f'RATING: {CusRA}', 
                (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255)
        ...
        self._sy.draw(
            700, 1570, 27, 
            f'Designed by Yuri-YuzuChaN & BlueDeer233 & TomeChen. Generated by {maiconfig.botName} BOT', 
        ...
        if self.normal:
            ...
            self.whiledraw(self.sdBest, False, 235)
...
async def generateb50(qqid: Optional[int] = None, username: Optional[str] = None) -> Union[MessageSegment, str]:
    """
    生成b50
    ...
    try:
        ...
        log.info(userinfo)
        draw_best = DrawBest(UserInfo = userinfo, qqid = qqid)
